...
- **`landing-page/diagnostic/`** — "What's Your Social Media Style?" free-tool diagnostic (built with the free-tool-builder skill). **LIVE:** https://landing-page-black-ten-44.vercel.app/diagnostic — for SMB owners running their own social. 5 lateral ungameable archetypes + A–F coherence grade + vanity-metric drift gate. Pure scoring module (`js/scoring.js`, 10 TDD tests), UI inherits site tokens, tiered opt-in funnel under a verbatim consent block. See `diagnostic/SPEC.md`.
  - **Storage LIVE:** Supabase project `studio-t-marketing-tools` (ref `lalrfkwrldlxvdedvplg`, us-east-1, free tier). 3 anon-INSERT-only tables (`diagnostic_submissions`/`_contacts`/`_events`); anon can insert, **cannot read** (no SELECT policy — reads via dashboard/service role only). The anon JWT in `js/app.js` is publishable by design. Kit uses the shared form `9547215`; call link → `studiot-creative.com/#contact`. Smoke-tested end-to-end (insert 201, read blocked) on 2026-06-24.
  - Note: Kit custom fields `archetype` + `grade` are sent on opt-in; they must exist in the Kit form or Kit silently drops them (email capture still works).
